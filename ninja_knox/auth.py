...
class BasicAuth(HttpBasicAuth):
    def authenticate(self, request, username, password):
        ...
        if user:
            if user_model.check_password(user, password):
                return user
            else:
                return False
        return False
...
class TokenAuth(HttpBearer):
    def authenticate(self, request, token):
        token_obj = (
            AuthToken.objects.filter(token=token).prefetch_related("user").first()
        )
        print(token_obj)
        if token_obj:
            # clean up expired tokens
            token_obj.user.auth_tokens.exclude(token=token).filter(
